...
#		print mask
	...
	def chmodDb(self):
		chmod = "777"
		self.printMessage("Setting permissions for " + self.name + " to" +
			Config.mpiblast_shared_chmod)
		invoke = ConfigSys.chmod
		if self.flag_debug:
			invoke += " -v"
		invoke += " " + chmod + " " + self.name + "*"
...
#			print os.environ["MPIBLAST_LOCAL"]
		...
		# remove
			...
			if (self.delete_filemask != None):
				...
				self.invokeCmd(invoke)
			if (not self.flag_skipreorder):
				# fasta was dumped separately, nuke fasta file
				invoke = ConfigSys.rm + " -rf"
				if (self.flag_debug):
					invoke += " -v"
				invoke += " " + self.name
