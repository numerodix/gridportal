...
class User:
	...
	def setDN(self, DN):
		...
		self.userDirorg = string.replace(self.DN.getOrganisationalUnit(), " ", "_")
		self.userDirorg2 = string.replace(self.DN.getOrganisation2(), " ", "_")
		
		# Create the directory
		
		self.userDir = os.getcwd()+"/%s/%s/%s" % (self.userDirorg2, self.userDirorg, self.userDirname)
	...
	def createDir(self):
		if self.DN!=None:
			if not os.path.exists(self.userDir):
				self.checkAndMkdir(os.getcwd()+"/%s" % self.userDirorg2)
				self.checkAndMkdir(os.getcwd()+"/%s/%s" % (self.userDirorg2, self.userDirorg))
				self.checkAndMkdir(os.getcwd()+"/%s/%s/%s" % (self.userDirorg2, self.userDirorg, self.userDirname))
				self.savePrefs()



	def exists(self):
		return os.path.isdir(os.getcwd()+"/%s/%s/%s" % (self.userDirorg2, self.userDirorg, self.userDirname))
