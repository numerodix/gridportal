...
class MatlabJobPage(JobPage):

	def onCreateNewTask(self):
	
		LapUtils.log.msg("MatlabJobPage", "onCreateNewTask")		

		return LapMatlab.MatlabTask()

	def onCreateNewJobForm(self, task):
	
		LapUtils.log.msg("MatlabJobPage", "onCreateNewJobForm")
		
		form = LapWeb.Form(self, "testform", "MatlabJobPage", "Create a Matlab job")
...
#		form.addText(os.getcwd(), "cwd", "")
	...
	def onCreateEditJobForm(self, task):
		...
		LapUtils.log.msg("MatlabJobPage", "onCreateEditJobForm")

		form = LapWeb.Form(self, "testform", "MatlabJobPage", "Edit Matlab job")
...
#		form.addText("License server", "licenseServer", attribs["licenseServer"])
	...
	def onValidate(self, request):
		...
		LapUtils.log.msg("MatlabJobPage", "onValidate")
...
#		field["licenseServer"] = self.getField(request, 'licenseServer')
	...
	def onHandleFileTransfers(self, request, destDir):
		...
		LapUtils.log.msg("MatlabJobPage", "onHandleFileTransfers")
	...
	def onAssignAttribs(self, task, field, request):
		...
		LapUtils.log.msg("MatlabJobPage", "onAssignTaskAttribs")
...
#		attribs["licenseServer"] = field["licenseServer"]
		...
		if self.request().hasField("inputFile"):
			...
			LapUtils.log.msg("MatlabJobPage", "onAssignTaskAttribs", "f.filename = %s" % filename)
