...
class CertDownloadPage(LunarcPage):
	def title(self):
		return 'File view'

	def writeHTML(self):
		
		fileEntry = self.session().value("certdownload_file")
		...
		self.session().setValue("nologin_download_complete")
		
		self.forward("CertRequestPage")
