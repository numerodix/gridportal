...
import Lap
import LapWeb
import PL_MpiBlastJob
import LapUtils

class PL_MpiBlastJobPage(JobPage):
	...
	def onCreateNewTask(self):
		...
		LapUtils.log.msg("PL_MpiBlastJobPage"+self.blastType, "onCreateNewTask")
		
		return PL_MpiBlastJob.MpiBlastTask(self.blastType)

	def onCreateNewJobForm(self, task):
	
		LapUtils.log.msg("PL_MpiBlastJobPage"+self.blastType, "onCreateNewJobForm")
		
		form = LapWeb.Form(self, "testform", "PL_MpiBlastJobPage"+self.blastType, "Create a "+self.blastType+" job")
	...
	def onCreateEditJobForm(self, task):
		...
		LapUtils.log.msg("PL_MpiBlastJobPage"+self.blastType, "onCreateEditJobForm")
	
		form = LapWeb.Form(self, "testform", "PL_MpiBlastJobPage"+self.blastType, "Edit "+self.blastType+" job")
	...
	def onValidate(self, request):
		...
		LapUtils.log.msg("PL_MpiBlastJobPage"+self.blastType, "onValidate")
	...
	def onHandleFileTransfers(self, request, destDir):
		...
		LapUtils.log.msg("PL_MpiBlastJobPage"+self.blastType, "onHandleFileTransfers")
	...
	def onAssignAttribs(self, task, field, request):
		...
		LapUtils.log.msg("PL_MpiBlastJobPage"+self.blastType, "onAssignTaskAttribs")
		...
		for i in range(0, len(atts)):
			if self.request().hasField(atts[i]):
				...
				LapUtils.log.msg("PL_MpiBlastJobPage"+self.blastType, "onAssignTaskAttribs", "f.filename = %s" % filename)
