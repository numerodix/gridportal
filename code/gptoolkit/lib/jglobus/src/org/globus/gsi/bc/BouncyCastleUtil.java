...
public class BouncyCastleUtil {
	...
	if (X509Name.CN.equals(ava.getObjectAt(0))) {
		...
		if (ext != null) {
			...
			throw new 
                            CertificateException("ProxyCertInfo extension must"
                                                 + " e critical");
		    }
		}
	    }
		...
		if (!issuer.equals(subject)) {
		    throw new CertificateException("Issuer name + proxy CN "
                                                   + "entry is not equal to "
                                                   + " subject name");
		}
	    }
	}
	...
	if (cert instanceof X509CertificateObject) {
	    ...
	    throw new IllegalArgumentException(
	      "Unexpected certificate type: " + cert.getClass()
            );
	}
    }
