...
public class ProxyPathValidatorTest extends TestCase {
	...
	certRevLists = 
	    CertificateRevocationLists.getCertificateRevocationLists("src" + 
				       ...
				       + ",src" + File.separator + BASE);
	try {
	    validator.validate(chain, new X509Certificate[] {goodCertsArr[21]},
			       certRevLists);
	} catch (ProxyPathValidatorException e) {
	    e.printStackTrace();
	    fail("Unexpected exception: " + e.getMessage());
	...
	try {
	    validator.validate(chain, new X509Certificate[] {goodCertsArr[25]},
			       certRevLists);
	    fail("Validation did not throw exception");
    ...
    // for testing only to disalbe validity checking
