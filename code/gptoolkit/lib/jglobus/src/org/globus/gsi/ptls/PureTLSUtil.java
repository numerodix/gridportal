...
public class PureTLSUtil {
	...
	while(!done) {
	    ...
	    if (pos1 == -1) {
		throw new Exception("Bad name, '=' missing in : " + globusID);
	    }
		...
		if (pos2 == -1) {
		    rdn = parseRDN(globusID.substring(off+1));
		    done = true;
		    break;
		} else {
			...
			if (pos3 <= pos1) {
			    ...
			    rdn = parseRDN(globusID.substring(off+1, pos3));
			    off = pos3;
			    break;
			}
		    } else {
	...
	if (pos == -1) {
	    rdn = new Vector(1);
	    rdn.addElement(getAVA(token));
	} else {
	    rdn = new Vector(2);
	    rdn.addElement(getAVA(token.substring(0, pos)));
	...
	int pos = rdn.indexOf('=');
	if (pos == -1) {
	    throw new Exception("Missing '=' in " + rdn);
	}
