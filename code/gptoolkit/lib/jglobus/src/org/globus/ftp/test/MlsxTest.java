...
public class MlsxTest extends TestCase {
	...
	*/
	...
	public void testMlsxEntry() throws Exception {
		...
		assertEquals(entry.get(entry.TYPE), entry.TYPE_FILE);
		assertEquals(entry.get(entry.PERM), "r");
		assertEquals(entry.get(entry.SIZE), "1024990");
		assertEquals(entry.get(entry.CREATE), null);

		entry = new MlsxEntry("Type=dir;Modify=19981107085215;Perm=el; /tmp");
		assertEquals(entry.getFileName(), "/tmp");
		assertEquals(entry.get(entry.TYPE), entry.TYPE_DIR);
		assertEquals(entry.get(entry.MODIFY), "19981107085215");

		entry = new MlsxEntry("Type=pdir;Modify=19990112030508;Perm=el; ..");
		assertEquals(entry.getFileName(), "..");
		assertEquals(entry.get(entry.TYPE), entry.TYPE_PDIR);
