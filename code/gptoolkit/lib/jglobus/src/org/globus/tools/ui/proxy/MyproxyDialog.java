 ...
 * MyproxyDialog.java
 *
 * A dialog to allow users to set MyroxyProperties
 *
 * $Id: MyproxyDialog.java,v 1.6 2005/04/18 15:36:46 gawor Exp $
 */
...
public class MyproxyDialog extends JDialog implements ActionListener {
  ...
  private JButton okButton, saveButton, exitButton; 
	...
	gridPanel.add(new JLabel("Myproxy Server: "),
		      0, 0, 1, 1);
	
	gridPanel.add(new JLabel("Myproxy Port: "),
		      0, 1, 1, 1);

	gridPanel.add(new JLabel("Portal Lifetime: "),
		      0, 2, 1, 1);

	gridPanel.add(new JLabel("Credential Lifetime: "),
		      0, 3, 1, 1);
	...
	gridPanel.add(myproxyServerTF,
		      1, 0, 1, 1);

	myproxyPortTF.setText("" + gridProps.getMyproxyPort());
	gridPanel.add(myproxyPortTF,
		      1, 1, 1, 1);


	contentPane.add(gridPanel, BorderLayout.CENTER);

	JPanel portalLifetimePanel = new JPanel(new FlowLayout(FlowLayout.LEFT,0,0));
	...
	gridPanel.add(portalLifetimePanel,
		      1, 2, 1, 1);
	
	JPanel credLifetimePanel = new JPanel(new FlowLayout(FlowLayout.LEFT,0,0));
	...
	gridPanel.add(credLifetimePanel,
		      1, 3, 1, 1);
  ...
  private boolean validateSettings() {
    ...
    gridProps.setMyproxyPort(new Integer(myproxyPortTF.getText().trim()).intValue());
    gridProps.setCredLifetime(new Integer (credLifetimeTF.getText().trim()).intValue());
    gridProps.setPortalLifetime(new Integer(portalLifetimeTF.getText().trim()).intValue());
